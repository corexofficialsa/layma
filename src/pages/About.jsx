            ...
            {globalTeam.map((member, i) => (
              <ScrollReveal key={member.id} delay={i * 0.1}>
                <motion.div whileHover={{ y: -6 }} style={{ background: 'rgba(241,242,196,0.07)', border: '1px solid rgba(186,191,38,0.2)', borderRadius: 24, overflow: 'hidden', textAlign: 'center' }}>
                  <div style={{ aspectRatio: '1', background: 'rgba(186,191,38,0.1)', overflow: 'hidden', position: 'relative', minHeight: 200 }}>
                    {member.image
                      ...
                      : <div style={{ height: '100%', minHeight: 200, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ width: 64, height: 64, borderRadius: '50%', background: 'rgba(186,191,38,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Cormorant Garamond, serif', fontSize: 28, color: '#BABF26' }}>{member.name.charAt(0)}</div></div>}
                  </div>
