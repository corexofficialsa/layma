        ...
        <div className="container">
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 14, justifyContent: 'center' }}>
            {exportTeam.map((member, i) => (
              <ScrollReveal key={member.id} delay={i * 0.1} style={{ width: 140 }}>
                <motion.div whileHover={{ y: -4 }} style={{ background: 'rgba(240,242,240,0.07)', border: '1px solid rgba(136,166,123,0.25)', borderRadius: 14, overflow: 'hidden', textAlign: 'center' }}>
                  <div style={{ aspectRatio: '1', background: 'rgba(136,166,123,0.15)', overflow: 'hidden' }}>
                    {member.image
                      ...
                      : <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ width: 42, height: 42, borderRadius: '50%', background: 'rgba(136,166,123,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Cormorant Garamond, serif', fontSize: 20, color: '#88A67B' }}>{member.name.charAt(0)}</div></div>}
                  </div>
                  <div style={{ padding: '12px 10px' }}>
                    <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 14, fontWeight: 600, color: '#F0F2F0', marginBottom: 4 }}>{member.name}</div>
                    <div style={{ fontSize: 8, letterSpacing: 1.2, textTransform: 'uppercase', color: '#88A67B', fontWeight: 600, lineHeight: 1.4 }}>{member.role}</div>
                  </div>
                </motion.div>
              </ScrollReveal>
            ))}
